...
<div>
    ...
    <c:forEach items="${leaveMsgs}" var="leaveMsg">
        ...
        <article class="comment">
            <section style="text-align:left">
                <%= i++  %>楼&nbsp;&nbsp;${leaveMsg.name}&nbsp;&nbsp;${leaveMsg.times}<br/><br/>
                <p>${leaveMsg.message}</p><br/>
            </section>
        </article>
    </c:forEach>
    <div class="form-horizontal" role="form" style="margin:10px">
        <div class="form-group">
            ...
            <div class="col-sm-3">
                <textarea id="message"  class="form-control" rows="3"  placeholder="文明上网，理性发言" ></textarea>
            </div>
        </div>
    ...
    <script>
        ...
        $("#commentButton").click(function () {
            if($("#message").val()==''&&$("#name").val()==''&&$("#email").val()==''){
                $("#info").text("提示:请输入留言内容,昵称和邮箱");
            }
            else if ($("#message").val()==''){
                $("#info").text("提示:请输入留言内容");
            }
            else if($("#name").val()==''){
                $("#info").text("提示:请输入昵称");
            }
    ...
    <div style="text-align: center">
        <ul class="pagination" >
            <li <c:if test="${pageInfo.pageNum==1}">class="disabled"</c:if>><a href="/api/messageboard/?page=1">&laquo;</a></li>
            <c:forEach begin="1" end="${pageInfo.pages}" step="1" var="pageNo">
                <li <c:if test="${pageInfo.pageNum==pageNo}">class="active"</c:if>><a href="/api/messageboard/?page=${pageNo}">${pageNo}</a></li>
            </c:forEach>
            <li <c:if test="${pageInfo.pageNum==pageInfo.pages}">class="disabled"</c:if>><a href="/api/messageboard/?page=${pageInfo.pages}">&raquo;</a></li>
        </ul>
    </div>
</div>
